...
export default function Sidebar({
  ...
  const handleDelete = async (e: React.MouseEvent, id: string) => {
    ...
    try {
      ...
      queryClient.setQueryData<HistoryItem[]>(historyKey, (prev = []) =>
  ...
  return (
    <>
      ...
      >
        ...
        {/* Logo area */}
        <div
          className={`p-4 flex items-center gap-3 border-b border-dark-700 ${!isOpen && "justify-center"}`}
        >
          <div onClick={() => navigate("/app")} className="cursor-pointer">
            <img
              src="/favicon.svg"
              alt="Logo"
              className="w-8 h-8 drop-shadow-[0_0_8px_rgba(6,182,212,0.4)]"
            />
          </div>
          {isOpen && (
        ...
        {/* User Profile Area */}
        <div
          className={`p-4 border-b border-dark-700 ${!isOpen && "flex justify-center"}`}
        >
          {user ? (
            ...
            <div className="flex flex-col gap-3">
              <p className="text-gray-400 text-xs px-1 leading-relaxed">
                Sign in to save search history and sync across devices.
              </p>
              <button
              ...
              {history.length > 0 ? (
                  ...
                  {history.map((item) => (
                    <div
                      ...
                      onClick={() => {
                        ...
                        onSelectTopic(
                          item.topic,
                          effectiveMode,
                          isLevel(firstLevel) ? firstLevel : undefined,
                        );
                      }}
                    ...
                    >
                      <div className="flex items-center gap-2.5 min-w-0 flex-1">
                        ...
                        <span
                          className={`text-[10px] font-bold px-1.5 py-0.5 rounded-md uppercase tracking-wider shrink-0 whitespace-nowrap ${
                            item.mode === "technical"
                ...
                <div className="px-2 py-4 space-y-2 animate-pulse">
                  {[1, 2, 3].map((i) => (
                    <div
                      key={i}
                      className="h-8 bg-dark-700 rounded-md w-full opacity-50"
                    ></div>
                  ))}
                </div>
              ) : (
            ...
            <div className="flex items-center gap-4 mb-4">
              ...
              <div>
                ...
                <p className="text-gray-400 text-sm mt-1">
                  This will permanently delete all your previous searches.
                </p>
              </div>
            </div>
