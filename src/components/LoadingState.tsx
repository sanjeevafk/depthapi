...
export function LoadingState({
  ...
  useEffect(() => {
    ...
    if (
      ...
      lowerTopic.includes("flow") ||
      lowerTopic.includes("sequence")
    ) {
    ...
    const fetchQuote = async () => {
      ...
      const cachedFresh =
        cachedFallback &&
        Date.now() - cachedFallback.savedAt < FALLBACK_CACHE_TTL_MS;
      ...
      try {
        const response = await fetch(
          "https://api.quotable.io/random?tags=education|knowledge|learning|science|wisdom|research|effort|creativity&maxLength=100",
          {
