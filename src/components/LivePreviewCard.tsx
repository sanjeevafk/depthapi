...
export function LivePreviewCard(): JSX.Element {
  ...
  // Animation constants
  const queryText =
    "Explain backpropagation like I know calculus but hate derivations.";
          ...
          <div className="flex-1 pt-2 space-y-4">
            {step >= 2 && (
              <div className="space-y-4">
                ...
                {step >= 3 &&
                  responseLines.map((lineTokens, i) => (
                    <ResponseLine
                      key={i}
                      tokens={lineTokens}
...
function ResponseLine({
  ...
  tokens: { text: string; type: "normal" | "highlight" }[];
  delay: number;
}): JSX.Element | null {
        ...
        <span
          ...
          className={
            token.type === "highlight"
              ? "text-cyan-200 font-semibold drop-shadow-[0_0_8px_rgba(34,211,238,0.3)]"
              : ""
          }
        >
          {token.text}
