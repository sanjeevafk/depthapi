-- Migration: 202605120001_add_trusted_corpus_search.sql
-- Adds a trusted-corpus hybrid search function without api_key scoping.

BEGIN;

DROP FUNCTION IF EXISTS public.hybrid_search_trusted_v4(TEXT, VECTOR(768), INT, INT, FLOAT, INT);

CREATE OR REPLACE FUNCTION public.hybrid_search_trusted_v4(
  query_text TEXT,
...
BEGIN
  ...
  fts_ranks AS (
    ...
    FROM knowledge_chunks kc
    JOIN knowledge_documents kd ON kc.document_id = kd.id
    WHERE kc.fts_tokens @@ websearch_to_tsquery(kd.language_config::regconfig, query_text)
      AND kc.deleted_at IS NULL
      AND kd.deleted_at IS NULL
    ORDER BY ts_rank_cd(kc.fts_tokens, websearch_to_tsquery(kd.language_config::regconfig, query_text)) DESC, kc.id
    LIMIT candidate_pool_size
