-- Migration: 202605160003_tune_rrf_weights_for_mode_separation.sql
-- Optimizes k-values in RRF calculation to ensure distinct separation between 
-- conceptual and technical retrieval modes.

BEGIN;

-- 1. Update primary hybrid_search_v5
...
BEGIN
  ...
  IF query_mode = 'code' OR query_mode = 'technical' THEN
    k_dense          := 100; -- Low impact of semantic
    k_sparse_simple  := 5;   -- HIGH priority for symbols
    k_sparse_english := 20;  -- High priority for technical keywords
  ELSE
    k_dense          := 5;   -- HIGH priority for conceptual meaning
    k_sparse_simple  := 100; -- Low impact of symbols
    k_sparse_english := 60;  -- Lower priority for exact lexical match
  END IF;
...
-- 2. Update trusted hybrid_search_trusted_v5
