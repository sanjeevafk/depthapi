-- Migration: 202605160005_optimize_retrieval_performance.sql
-- Optimizes hybrid search functions to avoid nested loop joins and resolve statement timeouts.
-- 1. Uses array-based collection filtering to reduce join complexity.
-- 2. Forces GIN index usage by decoupling permission checks from the primary index scan.
-- 3. Increases statement_timeout for the retrieval functions to 60s.

BEGIN;

-- 1. OPTIMIZED hybrid_search_v5
...
BEGIN
  -- Pre-fetch valid collection IDs for this API key to avoid redundant joins
  SELECT array_agg(id) INTO v_collection_ids
  FROM public.knowledge_collections
  WHERE api_key_id = target_api_key_id
    AND (target_collection_id IS NULL OR id = target_collection_id)
...
ALTER FUNCTION public.hybrid_search_v5(text, vector, uuid, text, int, int, float, uuid) SET statement_timeout = '60s';

-- 2. OPTIMIZED hybrid_search_trusted_v5
...
ALTER FUNCTION public.hybrid_search_trusted_v5(text, vector, text, int, int, float) SET statement_timeout = '60s';
