...
CREATE OR REPLACE FUNCTION hybrid_search_v5(query_text text, query_embedding vector(768), collection_filter uuid DEFAULT NULL, api_key_filter uuid DEFAULT NULL) RETURNS TABLE(content text, document_id uuid, source_url text, score real) LANGUAGE sql STABLE AS $$ WITH ranked AS (SELECT c.content, c.document_id, d.source_url, CASE WHEN c.embedding IS NULL THEN 0::real ELSE (1 - (c.embedding <=> query_embedding))::real END AS vector_score, ts_rank(c.fts_tokens, plainto_tsquery('english', query_text))::real AS lexical_score FROM knowledge_chunks c JOIN knowledge_documents d ON d.id = c.document_id JOIN knowledge_collections k ON k.id = d.collection_id WHERE api_key_filter IS NOT NULL AND k.api_key_id = api_key_filter AND (collection_filter IS NULL OR d.collection_id = collection_filter)) SELECT ranked.content, ranked.document_id, ranked.source_url, (0.7 * ranked.vector_score + 0.3 * ranked.lexical_score)::real FROM ranked ORDER BY (0.7 * ranked.vector_score + 0.3 * ranked.lexical_score) DESC LIMIT 8 $$;
