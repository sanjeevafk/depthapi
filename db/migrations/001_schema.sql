...
CREATE TABLE IF NOT EXISTS knowledge_collections (id uuid PRIMARY KEY, name text NOT NULL, description text, metadata jsonb NOT NULL DEFAULT '{}', created_at timestamptz NOT NULL DEFAULT now());
...
CREATE OR REPLACE FUNCTION hybrid_search_v5(query_text text, collection_filter uuid DEFAULT NULL) RETURNS TABLE(content text, document_id uuid, source_url text, score real) LANGUAGE sql STABLE AS $$ SELECT c.content, c.document_id, d.source_url, ts_rank(c.fts_tokens, plainto_tsquery('english', query_text)) FROM knowledge_chunks c JOIN knowledge_documents d ON d.id = c.document_id WHERE (collection_filter IS NULL OR d.collection_id = collection_filter) ORDER BY ts_rank(c.fts_tokens, plainto_tsquery('english', query_text)) DESC LIMIT 8 $$;
CREATE OR REPLACE FUNCTION hybrid_search_trusted_v5(query_text text, collection_filter uuid DEFAULT NULL) RETURNS TABLE(content text, document_id uuid, source_url text, score real) LANGUAGE sql STABLE AS $$ SELECT * FROM hybrid_search_v5(query_text, collection_filter) $$;
