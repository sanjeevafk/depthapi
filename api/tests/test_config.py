...
def test_provider_key_masking_in_repr_and_model_dump():
    ...
    assert dumped["openai_api_key"].get_secret_value() == raw_key
    assert raw_key not in str(dumped["openai_api_key"])


def test_provider_key_validator_accepts_secretstr_input():
    settings = Settings(openai_api_key=SecretStr("  secret-from-secretstr  "))
    assert settings.openai_api_key.get_secret_value() == "secret-from-secretstr"
