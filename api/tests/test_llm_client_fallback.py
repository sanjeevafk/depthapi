...
@pytest.mark.asyncio
async def test_create_chat_completion_records_provider_attempt_telemetry(monkeypatch):
    importlib.reload(llm_client_module)
    ...
    assert response.choices[0].message.content == "ok"
    assert attempts == ["groq", "gemini"]
    assert telemetry["requested_model"] == "default-fast"
    assert telemetry["actual_provider"] == "gemini"
    assert telemetry["provider_attempts"][0]["provider"] == "groq"
    assert telemetry["provider_attempts"][0]["outcome"] == "error"
    assert telemetry["provider_attempts"][1]["provider"] == "gemini"
    assert telemetry["provider_attempts"][1]["outcome"] == "success"
